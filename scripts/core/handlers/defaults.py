...
from scripts.config import Service, PathConf
...
class DefaultHandler:
    ...
    def load_styles():
        try:
            ...
            return DefaultSuccessResponse(message="Styles loaded successfully", data=response, status="success")
    ...
    def download_resources(filename: str):
        try:
            ...
            if os.path.isfile(os.path.join(Service.BUILD_DIR, filename)) and filename.endswith(".js"):  
                return FileResponse(file_path, media_type='text/javascript', filename=filename)  
